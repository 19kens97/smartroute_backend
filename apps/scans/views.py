...
def build_documents_style_payload(vehicle):
    ...
    latest_insurance = (
        InsurancePolicy.objects.filter(vehicle=vehicle)
    ...
    if request.FILES.get("image"):
        try:
            ...
            if exceptions is not None:
                if isinstance(e, exceptions.InvalidArgument):
                    return JsonResponse(
                        {"status": "error", "message": "Image invalide ou format non supporte."},
                        status=400,
                    )
                if isinstance(e, exceptions.Unauthenticated):
                    return JsonResponse(
                        {"status": "error", "message": "Configuration Gemini invalide (authentification)."},
                        status=502,
                    )
                if isinstance(e, exceptions.PermissionDenied):
                    return JsonResponse(
                        {"status": "error", "message": "Acces Gemini refuse. Verifiez la configuration du projet Google."},
                        status=502,
                    )
                if isinstance(e, exceptions.ResourceExhausted):
                    return JsonResponse(
                        {"status": "error", "message": "Quota Gemini depasse. Reessayez plus tard."},
                        status=429,
                    )
                if isinstance(e, exceptions.ServiceUnavailable):
                    return JsonResponse(
                        {"status": "error", "message": "Le service est temporairement surcharge. Reessayez dans quelques instants."},
                        status=503,
                    )
            ...
            if genai_errors is not None and isinstance(e, genai_errors.ServerError):
                ...
                return JsonResponse(
                    {"status": "error", "message": "Erreur de service Gemini pendant l'analyse de l'image."},
                    status=502,
                )

            logger.exception("Erreur inattendue pendant le scan de plaque Gemini.")
            return JsonResponse(
                {"status": "error", "message": "Erreur interne pendant l'analyse de l'image."},
                status=500,
            )
